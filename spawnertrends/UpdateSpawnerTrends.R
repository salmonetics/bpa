...
url_path <- "mongodb+srv://RUser:chinOOKsalmo*60@cluster0-aglp4.mongodb.net/test?retryWrites=true&w=majority"
m <- mongo(collection = "SpawnerAbundance", db = "Salmonetics",url = url_path,verbose = TRUE)
...
sps <- read.csv("SPS_Download_JUN082020.csv",stringsAsFactors=FALSE)
names(sps) <- c("Common_Population_Name","NMFS_POPID","NWR_Population_Name","Recovery_Domain","Major_Population_Group","ESU","Species","Run","Start_Year","End_Year","SPAWNINGYEAR","Spawners","FracWild","Age_1_Returns","Age_2_Returns","Age_3_Returns","Age_4_Returns","Age_5_Returns","Age_6_Returns","Age_7_Returns","Natural_Spawners_inc_Jacks","Total_Spawners_inc._Jacks","Total_Spawners_exc._Jacks","Jack_frac_of_hatchery_spawners","Adult_Natural_Harvest_Rates","Natural_Origin_Brood_Stk","CONTACTAGENCY")
sps <- merge(sps,pops %>% select(NMFS_POPID,POPID,SPECIES),all.x=T)
...
for(i in 1:nrow(bestdata)){
  ...
  m$update(query=bestdata$QUERY[i], '{"$set":{"timeseries": true}}', multiple = TRUE)
}

#test for one pop

m$find(query='{"POPID": 7}') %>% View()

#retrieve best from DB

testbest <- m$find(query='{"timeseries": true}')
